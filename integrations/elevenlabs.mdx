...
Create voice-based conversational AI agents with memory capabilities by integrating ElevenLabs and PhotoMem. This integration enables persistent, context-aware voice interactions that remember past conversations.

## Overview

In this guide, we'll build a voice agent that:
1. Uses ElevenLabs Conversational AI for voice interaction
2. Leverages PhotoMem to store and retrieve memories from past conversations
...
```bash
pip install elevenlabs photomem python-dotenv
```

Configure your environment variables:

<Note>You'll need both an ElevenLabs API key and a PhotoMem API key to use this integration.</Note>
...
- `AsyncMemoryClient` from PhotoMem for memory operations
...
def main():
    ...
    PHOTOMEM_API_KEY = os.environ.get('PHOTOMEM_API_KEY')
    ...
    if not PHOTOMEM_API_KEY:
        sys.stderr.write("PHOTOMEM_API_KEY environment variable must be set\n")
        sys.exit(1)

    # Set up PhotoMem API key in the environment
    os.environ['PHOTOMEM_API_KEY'] = PHOTOMEM_API_KEY
```
...
- Sets the PhotoMem API key in the environment for the PhotoMem client to use

### 3. Client Initialization

Initialize both the ElevenLabs and PhotoMem clients:
...
- Create an AsyncMemoryClient instance for PhotoMem interactions
...
- Stores the message in PhotoMem using the `add` method
...
When the agent identifies information worth remembering, it calls this function to store it in the PhotoMem database with the appropriate user ID.
...
4. The function stores the memory in PhotoMem linked to the user's ID
...
3. The function performs a semantic search in PhotoMem
...
By integrating ElevenLabs Conversational AI with PhotoMem, you can create voice agents that maintain context across conversations and provide personalized responses based on user history. This powerful combination enables:
