...
description: "Wire up PhotoMem OSS with your preferred LLM, vector store, embedder, and reranker."
icon: "sliders"
---

# Configure PhotoMem OSS Components
...
<Tabs>
  <Tab title="Python">
<Steps>
<Step title="Install PhotoMem OSS">
```bash
...
cd photomem/examples/docker-compose
```
</Step>
...
- `Unknown reranker` → update the SDK (`pip install --upgrade photomemai`) to load the latest provider registry.
