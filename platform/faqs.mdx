...
<AccordionGroup>
    ...
    <Accordion title="How do I configure PhotoMem for AWS Lambda?">
        ...
        To configure PhotoMem for AWS Lambda, set the `MEM0_DIR` environment variable to point to a writable directory in `/tmp`:

        ```bash
        MEM0_DIR=/tmp/.photomem
        ```
        ...
        photomem_dir = os.environ.get("MEM0_DIR") or os.path.join(home_dir, ".photomem")

        # To
        photomem_dir = os.environ.get("MEM0_DIR", "/tmp/.photomem")
        ```
    ...
    <Accordion title="How do I disable telemetry in PhotoMem?">
        To disable telemetry in PhotoMem, you can set the `MEM0_TELEMETRY` environment variable to `False`:
