...
description: "Teach PhotoMem the labels that matter to your team."
---

# Custom Categories

PhotoMem automatically tags every memory, but the default labels (travel, sports, music, etc.) may not match the names your app uses. Custom categories let you replace that list so the tags line up with your own wording.

<Info>
  **Use custom categories when…**
  - You need PhotoMem to tag memories with names your product team already uses.
...
- Ensure `PHOTOMEM_API_KEY` is set in your environment or pass it to the SDK constructor.
...
- **Automatic tags** — As new memories come in, PhotoMem picks the closest matches from your list and saves them in the `categories` field.
...
- The PhotoMem dashboard (Project → Memories) displays the custom labels in the Category column.
