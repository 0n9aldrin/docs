...
The Group Chat feature enables PhotoMem to process conversations involving multiple participants and automatically attribute memories to individual speakers. This allows for precise tracking of each participant's preferences, characteristics, and contributions in collaborative discussions, team meetings, or multi-agent conversations.

When you provide messages with participant names, PhotoMem automatically:
...
PhotoMem automatically detects group chat scenarios when messages contain a `name` field:
...
When names are present, PhotoMem:
