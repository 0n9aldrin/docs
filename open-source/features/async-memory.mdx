---
title: Async Memory
description: Run PhotoMem operations without blocking your event loop.
icon: "bolt"
---

`AsyncMemory` gives you a non-blocking interface to PhotoMem’s storage layer so Python applications can add, search, and manage memories directly from async code. Use it when you embed PhotoMem inside FastAPI services, background workers, or any workflow that relies on `asyncio`.

<Info>
  **You’ll use this when…**
  - Your agent already runs in an async framework and you need memory calls to await cleanly.
  - You want to embed PhotoMem’s storage locally without sending requests through the synchronous client.
...
from photomem.configs.base import MemoryConfig
