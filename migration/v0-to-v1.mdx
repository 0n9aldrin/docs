...
description: 'Complete guide to upgrade your PhotoMem implementation to version 1.0.0 '
icon: "arrow-right"
iconType: "solid"
---

<Warning>
**Breaking Changes Ahead!** PhotoMem 1.0.0  introduces several breaking changes. Please read this guide carefully before upgrading.
</Warning>

## Overview

PhotoMem 1.0.0  is a major release that modernizes the API, improves performance, and adds powerful new features. This guide will help you migrate your existing v0.x implementation to the new version.
...
pip install --upgrade photomemai
```
...
**Error:**
```
ValueError: The v1.0 API format is no longer supported in photomemai 1.0.0+
```
...
pip install photomemai==0.1.20  # Replace with your last working version
```
...
import photomem_v0  # Your old version
import photomem     # New version

def compare_results(query, user_id):
    old_results = photomem_v0.search(query, user_id=user_id)
    new_results = photomem.search(query, user_id=user_id)
...
<Info>
Need help with migration? Check our [GitHub Discussions](https://github.com/photomemai/photomem/discussions) or reach out to our community for support.
</Info>