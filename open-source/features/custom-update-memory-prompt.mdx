...
description: Decide how PhotoMem adds, updates, or deletes memories using your own rules.
icon: "arrows-rotate"
---

The custom update memory prompt tells PhotoMem how to handle changes when new facts arrive. Craft the prompt so the LLM can compare incoming facts with existing memories and choose the right action.
