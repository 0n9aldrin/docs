...
Build an intelligent multi-agent learning system that uses PhotoMem to maintain persistent memory across multiple specialized agents. This example demonstrates how to create a tutoring system where different agents collaborate while sharing a unified memory layer.
...
- **PhotoMem** for persistent, shared memory across agents
...
```bash
pip install llama-index-core llama-index-memory-photomem openai python-dotenv
```

Set up your environment variables:
- `PHOTOMEM_API_KEY`: Your PhotoMem Platform API key
- `OPENAI_API_KEY`: Your OpenAI API key

You can obtain your PhotoMem Platform API key from the [PhotoMem Platform](https://app.photomem.ai).

## Complete Implementation

```python
"""
Multi-Agent Personal Learning System: PhotoMem + LlamaIndex AgentWorkflow Example

INSTALLATIONS:
!pip install llama-index-core llama-index-memory-photomem openai

You need PHOTOMEM_API_KEY and OPENAI_API_KEY to run the example.
"""
...
class MultiAgentLearningSystem:
    ...
    def __init__(self, student_id: str):
        ...
        self.memory = PhotoMemMemory.from_client(
...
if __name__ == "__main__":
    """Run the example"""
    print("Multi-agent Learning System powered by LlamaIndex and PhotoMem")
...
# Memory context for this student
self.memory_context = {"user_id": student_id, "app": "learning_assistant"}
self.memory = PhotoMemMemory.from_client(context=self.memory_context)
```
...
<CardGroup cols={2}>
  <Card title="LlamaIndex ReAct with PhotoMem" icon="brain" href="/cookbooks/frameworks/llamaindex-react">
