...
1. Store and retrieve memories from PhotoMem within Google ADK agents
...
Before setting up PhotoMem with Google ADK, ensure you have:
...
The following example demonstrates how to create a Google ADK agent with PhotoMem memory integration:
...
def save_memory(content: str, user_id: str) -> dict:
    ...
    try:
        result = photomem.add([{"role": "user", "content": content}], user_id=user_id, output_format='v1.1')
