...
Multimodal support lets PhotoMem extract facts from images alongside regular text. Add screenshots, receipts, or product photos and PhotoMem will store the insights as searchable memories so agents can recall them later.
...
- **Vision processing:** PhotoMem runs the image through a vision model that extracts text and key details.
...
```python Python
from photomem import Memory
from photomem.exceptions import InvalidImageError, FileSizeError
