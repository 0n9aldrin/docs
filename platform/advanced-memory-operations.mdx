...
```bash
pip install "photomem[async]"
```
</Step>
<Step title="Export your API key">
```bash
export PHOTOMEM_API_KEY="sk-platform-..."
```
</Step>
...
memory = AsyncMemoryClient(api_key=os.environ["PHOTOMEM_API_KEY"])
```
</Step>
</Steps>
  </Tab>
...
const memory = new Memory({ apiKey: process.env.PHOTOMEM_API_KEY!, async: true });
```
</Step>
</Steps>
  </Tab>
...
- `Missing required key enableGraph`: update the SDK to `photomem>=0.4.0`.
