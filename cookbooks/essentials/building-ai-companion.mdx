---
title: Build a Companion with PhotoMem
...
The solution: PhotoMem. It extracts and stores what matters from conversations, then retrieves it when needed. Your companion remembers user preferences, past events, and history.
...
from photomem import MemoryClient

openai_client = OpenAI(api_key="your-openai-key")
photomem_client = MemoryClient(api_key="your-photomem-key")

def chat(user_input, user_id):
    # Retrieve relevant memories
    memories = photomem_client.search(user_input, user_id=user_id, limit=5)
...
# Stored in PhotoMem: "Max wants to run sub-4 marathon"
...
Ray remembers. Restart the app, and the goal persists. From here on, we'll focus on just the PhotoMem API calls.
...
- **Categories**: AI-assigned by PhotoMem based on content (you can't force them)
- **Metadata**: Manually set by you for forced tagging

Define custom categories at the project level. PhotoMem will automatically tag memories with relevant categories based on content:

```python
photomem_client.project.update(custom_categories=[
...
<Note>
**Categories vs Metadata:** Categories are AI-assigned by PhotoMem based on content semantics. You define the palette, PhotoMem picks which ones apply. If you need guaranteed tagging, use `metadata` instead.
</Note>

Now when you add memories, PhotoMem automatically assigns the appropriate categories:

```python
# Add goal - PhotoMem automatically tags it as "goals"
photomem_client.add(
    [{"role": "user", "content": "Sub-4 marathon is my A-race"}],
    user_id="max"
)

# Add constraint - PhotoMem automatically tags it as "constraints"
photomem_client.add(
...
PhotoMem reads the content and intelligently picks which categories apply. You define the palette, it handles the tagging.

**Important:** You cannot force specific categories. PhotoMem's platform decides which categories are relevant based on content. If you need to force-tag something, use `metadata` instead:
...
```python
constraints = photomem_client.search(
    "injury concerns",
    user_id="max",
...
```python
memories = photomem_client.get_all(user_id="max")
...
<Warning>
Without filters, PhotoMem stores everything—greetings, filler, and casual chat. This pollutes retrieval: instead of pulling "marathon goal," you get "lol ok." Set custom instructions to keep memory clean.
</Warning>
...
Tell PhotoMem what matters:

```python
photomem_client.project.update(custom_instructions="""
...
agent_memories = photomem_client.search("coaching style", agent_id="ray_coach")
...
### When to Store in PhotoMem

Don't send every single message to PhotoMem. Keep recent context in memory, let PhotoMem handle the important long-term facts.

```python
# Store only meaningful exchanges in PhotoMem
photomem_client.add([
...
Last 10 messages in your app's buffer. Important facts in PhotoMem. Faster, cheaper, still works.
...
Here's the PhotoMem setup combining everything:
...
photomem_client.project.update(
    custom_instructions="""
...
```python
memories = photomem_client.search("training plan", user_id="max", limit=5)
...
```python
photomem_client.add(messages, user_id="max", run_id="boston-2025")
photomem_client.add(messages, user_id="max", run_id="nyc-2025")

# Retrieve only Boston memories
boston_memories = photomem_client.search(
    "training plan",
    user_id="max",
    run_id="boston-2025"
)
...
for log in old_logs:
    photomem_client.add(log, user_id="max")
...
# Update it
photomem_client.update(goal_memory["id"], "Max wants to run sub-3:45 marathon")
...
speed_sessions = photomem_client.search(
    "speed work",
    user_id="max",
...
```python
photomem_client.delete(memory_id="mem_xyz")

# Or clear an entire run_id
photomem_client.delete_all(user_id="max", run_id="old-training-cycle")
...
- **Persists across sessions** - PhotoMem storage
...
<Tip>
Start with 2-3 categories max (e.g., goals, constraints, preferences). More categories dilute tagging accuracy. You can always add more later after seeing what PhotoMem extracts.
</Tip>
...
- Monitor memory quality in PhotoMem dashboard
