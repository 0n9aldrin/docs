---
title: 'Pipecat'
description: 'Integrate PhotoMem with Pipecat for conversational memory in AI agents'
---

# Pipecat Integration

PhotoMem seamlessly integrates with [Pipecat](https://pipecat.ai), providing long-term memory capabilities for conversational AI agents. This integration allows your Pipecat-powered applications to remember past conversations and provide personalized responses based on user history.

## Installation

To use PhotoMem with Pipecat, install the required dependencies:

```bash
pip install "pipecat-ai[photomem]"
```

You'll also need to set up your PhotoMem API key as an environment variable:

```bash
export MEM0_API_KEY=your_photomem_api_key
```

You can obtain a PhotoMem API key by signing up at [photomem.ai](https://photomem.ai).

## Configuration

PhotoMem integration is provided through the `PhotoMemMemoryService` class in Pipecat. Here's how to configure it:

```python
from pipecat.services.photomem import PhotoMemMemoryService

memory = PhotoMemMemoryService(
    api_key=os.getenv("MEM0_API_KEY"),  # Your PhotoMem API key
...
The `PhotoMemMemoryService` should be positioned between your context aggregator and LLM service in the Pipecat pipeline:
...
pipeline = Pipeline([
    ...
    memory,             # PhotoMem Memory service enhances context here
...
Here's a complete example of a Pipecat voice agent with PhotoMem memory integration:
...
When integrated with Pipecat, PhotoMem provides two key functionalities:

### 1. Message Storage

All conversation messages are automatically stored in PhotoMem for future reference:
...
2. Relevant memories are retrieved from PhotoMem's database
...
memory = PhotoMemMemoryService(
    ...
    params={
        ...
        "api_version": "v2",          # PhotoMem API version
    }
)
```
...
- [PhotoMem Pipecat Integration](https://docs.pipecat.ai/server/services/memory/photomem)
