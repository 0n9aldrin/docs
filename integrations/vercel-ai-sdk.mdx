...
The [**PhotoMem AI SDK Provider**](https://www.npmjs.com/package/@photomem/vercel-ai-provider) is a library developed by **PhotoMem** to integrate with the Vercel AI SDK. This library brings enhanced AI interaction capabilities to your applications by introducing persistent memory functionality.

<Note type="info">
  PhotoMem AI SDK now supports <strong>Vercel AI SDK V5</strong>.
</Note>
...
```bash
npm install @photomem/vercel-ai-provider
```
...
1. Get your **PhotoMem API Key** from the [PhotoMem Dashboard](https://app.photomem.ai/dashboard/api-keys).

2. Initialize the PhotoMem Client in your application:

    ```typescript
    import { createPhotoMem } from "@photomem/vercel-ai-provider";

    const photomem = createPhotoMem({
      provider: "openai",
      photomemApiKey: "m0-xxx",
      apiKey: "provider-api-key",
      config: {
        // Options for LLM Provider
      },
      // Optional PhotoMem Global Config
      photomemConfig: {
        user_id: "photomem-user-id",
      },
    });
    ```

    > **Note**: The `openai` provider is set as default. Consider using `PHOTOMEM_API_KEY` and `OPENAI_API_KEY` as environment variables for security.

    > **Note**: The `photomemConfig` is optional. It is used to set the global config for the PhotoMem Client (eg. `user_id`, `agent_id`, `app_id`, `run_id`, `org_id`, `project_id` etc).

3. Add Memories to Enhance Context:

    ```typescript
    import { LanguageModelV2Prompt } from "@ai-sdk/provider";
    import { addMemories } from "@photomem/vercel-ai-provider";
...
### Standalone Features:
    ...
    ```typescript
    await addMemories(messages, { user_id: "borat", photomemApiKey: "m0-xxx" });
    await retrieveMemories(prompt, { user_id: "borat", photomemApiKey: "m0-xxx" });
    await getMemories(prompt, { user_id: "borat", photomemApiKey: "m0-xxx" });
    ```
     > For standalone features, such as `addMemories`, `retrieveMemories`, and `getMemories`, you must either set `PHOTOMEM_API_KEY` as an environment variable or pass it directly in the function call.
...
### 1. Basic Text Generation with Memory Context
    ...
    const { text } = await generateText({
      model: photomem("gpt-4-turbo", { user_id: "borat" }),
...
### 2. Combining OpenAI Provider with Memory Utils
    ...
    import { retrieveMemories } from "@photomem/vercel-ai-provider";
...
### 4. Generate Responses with Tools Call
    ...
    const result = await generateText({
      model: photomem('claude-3-5-sonnet-20240620'),
      tools: {
        weather: tool({
...
PhotoMem AI SDK supports file processing with memory context. Here's an example of analyzing a PDF file:
...
async function main() {
  ...
  const { textStream } = streamText({
    model: photomem("gemini-2.5-flash"),
    messages: [
      {
        role: 'user',
        content: [
...
PhotoMem AI SDK now supports Graph Memory. You can enable it by setting `enable_graph` to `true` in the `photomemConfig` object.

```typescript
const photomem = createPhotoMem({
  photomemConfig: { enable_graph: true },
});
```
...
```typescript
const memories = await getMemories(prompt, { user_id: "borat", photomemApiKey: "m0-xxx", enable_graph: true });
```
...
- `createPhotoMem()`: Initializes a new PhotoMem provider instance.
...
PhotoMem's Vercel AI SDK enables the creation of intelligent, context-aware applications with persistent memory and seamless integration.

<CardGroup cols={2}>
  <Card title="OpenAI Agents SDK" icon="cube" href="/integrations/openai-agents-sdk">
    Build agents with OpenAI SDK and PhotoMem
  </Card>
