...
description: "Expose PhotoMem memories as callable tools inside OpenAI agent workflows."
---


Integrate PhotoMem's memory capabilities with OpenAI's Agents SDK to create AI agents with persistent memory. You can create agents that remember past conversations and use that context to provide better responses.

## Installation

First, install the required packages:
```bash
pip install photomem pydantic openai-agents
```
...
Store your PhotoMem API key as an environment variable:
...
```python
import os
os.environ["PHOTOMEM_API_KEY"] = "your_photomem_api_key"
```
...
try:
    ...
    raise ImportError("photomem is not installed. Please install it using 'pip install photomem'.")
from agents import (
    Agent,
    ItemHelpers,
    MessageOutputItem,
...
```python
class PhotoMemContext(BaseModel):
    user_id: str | None = None
```

### 3. Initialize the PhotoMem Client

```python
client = AsyncMemoryClient(api_key=os.getenv("PHOTOMEM_API_KEY"))
```
...
async def add_to_memory(
    context: RunContextWrapper[PhotoMemContext],
    content: str,
) -> str:
    """
    Add a message to PhotoMem
    Args:
...
async def search_memory(
    ...
    """
    Search for memories in PhotoMem
    Args:
...
async def get_all_memory(
    ...
    """Retrieve all memories from PhotoMem"""
    user_id = context.context.user_id or "default_user"
...
```python
memory_agent = Agent[PhotoMemContext](
...
```python
async def main():
    current_agent: Agent[PhotoMemContext] = memory_agent
    input_items: list[TResponseInputItem] = []
    context = PhotoMemContext()
    while True:
...
```python
context = PhotoMemContext(user_id="user123")
```


## Resources

- [PhotoMem Documentation](https://docs.photomem.ai)
- [PhotoMem Dashboard](https://app.photomem.ai/dashboard)
- [API Reference](https://docs.photomem.ai/api-reference)

---

<CardGroup cols={2}>
  <Card title="OpenAI Tool Calls with PhotoMem" icon="wrench" href="/cookbooks/integrations/openai-tool-calls">
