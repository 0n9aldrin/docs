...
description: "Wire PhotoMem memories into OpenAI's inbuilt function-calling flow."
---


Integrate PhotoMem’s memory capabilities with OpenAI’s Inbuilt Tools to create AI agents with persistent memory.
...
```bash
npm install photomem openai zod
```

## Environment Setup

Save your PhotoMem and OpenAI API keys in a `.env` file:
...
Get your PhotoMem API key from the [PhotoMem Dashboard](https://app.photomem.ai/dashboard/api-keys).

### Configuration

```javascript
const photomemConfig = {
    apiKey: process.env.PHOTOMEM_API_KEY,
    user_id: "sample-user",
};

const openAIClient = new OpenAI();
const photomemClient = new MemoryClient(photomemConfig);
```
...
```javascript
const relevantMemories = await photomemClient.search(userInput, photomemConfig);
```
...
async function main(memory = false) {
  ...
  if (memory) {
    relevantMemories = await photomemClient.search(input, photomemConfig);
  }
...
<CardGroup cols={2}>
  <Card title="Agents SDK Tool with PhotoMem" icon="robot" href="/cookbooks/integrations/agents-sdk-tool">
    Extend the OpenAI Agents SDK with PhotoMem integration capabilities.
  </Card>
