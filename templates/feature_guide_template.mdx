...
description: "Structure for explaining when and why to use a PhotoMem feature."
icon: "sparkles"
---

# Feature Guide Template

Use this when you introduce or deepen a single PhotoMem capability (Graph Memory, Advanced Retrieval, etc.). Aim for crisp problem framing, a walkthrough of how the feature works, and practical configuration guidance with clear exits.
...
- Add a `## Configure access` snippet (even if it’s “Confirm your PhotoMem API key is already configured”) so contributors never forget to mention the baseline setup.
...
PhotoMem’s advanced retrieval elevates search accuracy when basic keyword matches aren’t enough. Turn it on when you need precise context for high-stakes conversations.
...
```ts TypeScript
const memories = await photomem.memories.search({ criteria: { ... } });
```
</CodeGroup>
...
<CardGroup cols={2}>
  <Card title="Dive Into Memory Scoring" icon="scale-balanced" href="/concepts/memory-scoring">
    Understand how PhotoMem ranks memories under the hood.
  </Card>
