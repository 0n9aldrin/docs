...
| `keyspace` | Keyspace name | `"photomem"` |
...
docker run --name photomem-cassandra \
    -p 9042:9042 \
    -e CASSANDRA_CLUSTER_NAME="PhotoMemCluster" \
    -d cassandra:latest

# Wait for Cassandra to start (may take 1-2 minutes)
docker exec -it photomem-cassandra cqlsh

# Create keyspace
CREATE KEYSPACE IF NOT EXISTS photomem
...
config = {
    "vector_store": {
        ...
        "config": {
            ...
            "port": 9042,
            "username": "photomem_user",
            "password": "secure_password",
            "keyspace": "photomem_prod",
            "collection_name": "memories",
            "protocol_version": 4,
