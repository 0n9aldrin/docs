...
Reranker-enhanced search adds a second scoring pass after vector retrieval so PhotoMem can return the most relevant memories first. Enable it when keyword similarity alone misses nuance or when you need the highest-confidence context for an agent decision.
...
- **Reordered results:** PhotoMem sorts responses using the reranker’s scores before returning them.
