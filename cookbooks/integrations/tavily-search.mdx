...
description: "Blend Tavily's realtime results with personal context stored in PhotoMem."
---
...
That's what we are going to build today, a Personalized Search Assistant powered by PhotoMem for memory and [Tavily](https://tavily.com) for real-time search.
...
- With PhotoMem, your assistant builds a memory of the user's world.
...
```bash
pip install langchain photomem langchain-tavily langchain-openai
```
...
PHOTOMEM_API_KEY=your-photomem-key
```

## Code Walkthrough
Let’s break down the main components.

### 1: Initialize PhotoMem with Custom Instructions

We configure PhotoMem with custom instructions that guide it to infer user memories tailored specifically for our usecase.

```python
from photomem import MemoryClient

photomem_client = MemoryClient()
...
```
Now, if a user casually mentions "I need to pick up my daughter" or "What's the weather at Los Angeles", PhotoMem remembers they have a daughter or the user is interested in or connected with Los Angeles in terms of location. These details will be referenced for future searches.
...
def setup_user_history(user_id):
    ...
    for conversation in conversations:
        photomem_client.add(conversation, user_id=user_id)
```
...
def get_user_context(user_id, query):
    ...
    user_memories = photomem_client.search(query=query, filters=filters)
...
def store_search_interaction(user_id, original_query, agent_response):
    ...
    photomem_client.add(messages=interaction, user_id=user_id)
```
...
With PhotoMem and Tavily, you can build a search assistant that doesn't just fetch results but understands the person behind the query.

Whether for shopping, travel, or daily life, this approach turns a generic search into a truly personalized experience.

Full Code: [Personalized Search GitHub](https://github.com/photomemai/photomem/blob/main/examples/misc/personalized_search.py)

---

<CardGroup cols={2}>
  <Card title="Deep Research with PhotoMem" icon="magnifying-glass" href="/cookbooks/operations/deep-research">
