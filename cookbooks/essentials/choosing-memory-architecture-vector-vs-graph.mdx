...
PhotoMem brings graph memory into the picture to fill this gap. In this cookbook, we'll create a company knowledge base with PhotoMem, using both vector and graph stores. You'll learn when each one helps along the way.

---

## Vector and Graph Stores

When you add a memory to PhotoMem, it goes into a **vector store** by default. Vector stores are excellent at semantic search - finding memories that match the meaning of your query.
...
When you set `enable_graph=True`, PhotoMem extracts entities and relationships:
...
Graph memory adds processing time and cost. When you call `client.add()` with `enable_graph=True`, PhotoMem makes extra LLM calls to extract entities and relationships.
