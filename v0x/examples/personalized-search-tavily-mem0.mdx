...
```
Now, if a user casually mentions "I need to pick up my daughter", or "What's the weather at Los Angeles", PhotoMem remembers they have a daughter or user is somewhat interested/connected with Los Angeles in terms of location, those will be referred for future searches.
...
def setup_user_history(user_id):
    ...
    for conversation in conversations:
        photomem_client.add(conversation, user_id=user_id, output_format="v1.1")
```
...
def get_user_context(user_id, query):
    ...
    user_memories = photomem_client.search(query=query, version="v2", filters=filters)
...
def store_search_interaction(user_id, original_query, agent_response):
    ...
    photomem_client.add(messages=interaction, user_id=user_id, output_format="v1.1")
```
...
## Conclusion
With PhotoMem + Tavily, you can build a search assistant that doesn’t just fetch results but it understands the person behind the query.
