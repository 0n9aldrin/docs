...
description: Reach every PhotoMem OSS capability through a FastAPI-powered REST layer.
icon: "code"
---

The PhotoMem REST API server exposes every OSS memory operation over HTTP. Run it alongside your stack to add, search, update, and delete memories from any language that speaks REST.

<Info>
  **You’ll use this when…**
  - Your services already talk to REST APIs and you want PhotoMem to match that style.
  - Teams on languages without the PhotoMem SDK still need access to memories.
...
```bash
docker pull photomem/photomem-api-server
```
  </Tab>
  <Tab title="Build locally">
```bash
docker build -t photomem-api-server .
```
  </Tab>
</Tabs>
...
```bash
docker run -p 8000:8000 --env-file .env photomem-api-server
```
