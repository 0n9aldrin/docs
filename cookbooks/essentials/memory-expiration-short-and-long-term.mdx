...
By default, PhotoMem memories persist forever. This works for user preferences and core facts, but temporary data should expire automatically.
...
<Info icon="check">
**Expected behavior:** After 7 days, the session context automatically disappears—no cron jobs, no manual cleanup. The preference persists forever. PhotoMem handles expiration transparently.
</Info>

Memories with `expiration_date` are automatically removed after expiring. No cleanup job needed - PhotoMem handles it.
...
Memory expiration keeps storage clean and search results relevant. Use **`expiration_date`** for temporary data (session context, recent chats), skip it for permanent facts (preferences, account info). PhotoMem handles cleanup automatically—no background jobs required.
