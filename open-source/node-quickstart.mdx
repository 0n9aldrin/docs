...
description: "Store and search PhotoMem memories from a TypeScript or JavaScript app in minutes."
icon: "js"
---

Spin up PhotoMem with the Node SDK in just a few steps. You’ll install the package, initialize the client, add a memory, and confirm retrieval with a single search.
...
PhotoMem offers granular configuration across vector stores, LLMs, embedders, and history stores.
...
| Parameter | Description | Default |
| --- | --- | --- |
| `historyDbPath` | Path to history database | `"{photomem_dir}/history.db"` |
...
<CardGroup cols={3}>
  ...
  <Card title="Automate Node Workflows" icon="plug" href="/cookbooks/integrations/openai-tool-calls">
    See a full Node-based workflow that layers PhotoMem memories onto tool-calling agents.
  </Card>
</CardGroup>
