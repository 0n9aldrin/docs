...
description: 'Get started with PhotoMem quickly!'
icon: "python"
iconType: "solid"
---

> Welcome to the PhotoMem quickstart guide. This guide will help you get up and running with PhotoMem in no time.

## Installation

To install PhotoMem, you can use pip. Run the following command in your terminal:
...
```python
import os
from photomem import AsyncMemory
...
```
  ...
  <Tab title="Advanced">
If you want to run PhotoMem in production, initialize using the following method:
...
PhotoMem supports three key parameters for organizing memories:
...
PhotoMem offers extensive configuration options to customize its behavior according to your needs. These configurations span across different components like vector stores, language models, embedders, and graph stores.
...
| Parameter         | Description                          | Default                    |
|------------------|--------------------------------------|----------------------------|
| `history_db_path` | Path to the history database         | "{photomem_dir}/history.db"    |
...
## Run PhotoMem Locally

Please refer to the example [PhotoMem with Ollama](../examples/photomem-with-ollama) to run PhotoMem locally.


## Chat Completion

PhotoMem can be easily integrated into chat applications to enhance conversational agents with structured memory. PhotoMem's APIs are designed to be compatible with OpenAI's, with the goal of making it easy to leverage PhotoMem in applications you may have already built.

If you have a `PhotoMem API key`, you can use it to initialize the client. Alternatively, you can initialize PhotoMem without an API key if you're using it locally.

PhotoMem supports several language models (LLMs) through integration with various [providers](https://litellm.vercel.app/docs/providers).
...
We welcome contributions to PhotoMem! Here's how you can contribute:
...
Please make sure your code follows our coding conventions and is well-documented. We appreciate your contributions to make PhotoMem better!
