...
```bash
pip install photomemai pydantic openai-agents
```
...
```python
import os
os.environ["MEM0_API_KEY"] = "your_photomem_api_key"
```
...
try:
    ...
    raise ImportError("photomem is not installed. Please install it using 'pip install photomemai'.")
from agents import (
    Agent,
    ItemHelpers,
    MessageOutputItem,
