---
title: "PhotoMem with OpenAI Agents SDK for Voice"
description: "Integrate memory capabilities into your voice agents using PhotoMem and OpenAI Agents SDK"
---

# Building Voice Agents with Memory using PhotoMem and OpenAI Agents SDK

This guide demonstrates how to combine OpenAI's Agents SDK for voice applications with PhotoMem's memory capabilities to create a voice assistant that remembers user preferences and past interactions.
...
2. Installed PhotoMem SDK:

```bash
pip install photomemai
```
...
4. Set up your API keys:
   ...
   - PhotoMem API key from the PhotoMem Platform
...
# PhotoMem imports
from photomem import AsyncMemoryClient
...
# Initialize PhotoMem client
photomem_client = AsyncMemoryClient()
```

This section handles:

- Importing required modules from OpenAI Agents SDK and PhotoMem
- Setting up environment variables for API keys
- Defining a simple user identification system (using a global variable)
- Initializing the PhotoMem client that will handle memory operations
...
async def save_memories(
    ...
    # Store the preference in PhotoMem
    memory_content = f"User memory - {memory}"
    await photomem_client.add(
        memory_content,
        user_id=USER_ID,
    )
...
- Stores it in PhotoMem using the `add()` method
...
async def search_memories(
    ...
    results = await photomem_client.search(
        query,
        user_id=USER_ID,
        limit=5,
...
- Passes it to PhotoMem's semantic search to find related memories
...
Now that we've explained each component, here's the complete implementation that combines OpenAI Agents SDK for voice with PhotoMem's memory capabilities:
...
# Create tools that utilize PhotoMem's memory
@function_tool
async def save_memories(
    memory: str
) -> str:
...
async def main():
    ...
    try:
        while True:
            ...
            # Example of saving the conversation to PhotoMem after completion
            if agent_response:
                try:
                    await photomem_client.add(
                        f"Agent response: {agent_response}",
...
4. **Memory Management Tools**:
   - `save_memories`: Stores user memories in PhotoMem
...
The agent will listen to your request, process it through the OpenAI model, utilize PhotoMem for memory operations as needed, and respond both through text output and voice speech.
...
By combining OpenAI's Agents SDK with PhotoMem's memory capabilities, you can create voice agents that maintain persistent memory of user preferences and past interactions. This significantly enhances the user experience by making conversations more natural and personalized.
