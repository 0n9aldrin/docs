...
This guide demonstrates how to create a memory-enabled voice assistant using LiveKit, Deepgram, OpenAI, and PhotoMem, focusing on creating an intelligent, context-aware travel planning agent.
...
### 2. PhotoMem Client and Agent Definition

```python
# User ID for RAG data in PhotoMem
RAG_USER_ID = "livekit-photomem"
photomem_client = AsyncMemoryClient()

class MemoryEnabledAgent(Agent):
    """
    An agent that can answer questions using RAG (Retrieval Augmented Generation) with PhotoMem.
    """
    def __init__(self) -> None:
        ...
        logger.info(f"PhotoMem Agent initialized. Using user_id: {RAG_USER_ID}")
    ...
    async def on_user_turn_completed(self, turn_ctx: ChatContext, new_message: ChatMessage) -> None:
        # Persist the user message in PhotoMem
        try:
            logger.info(f"Adding user message to PhotoMem: {new_message.text_content}")
            add_result = await photomem_client.add(
                [{"role": "user", "content": new_message.text_content}],
                user_id=RAG_USER_ID
            )
            logger.info(f"PhotoMem add result (user): {add_result}")
        except Exception as e:
            logger.warning(f"Failed to store user message in PhotoMem: {e}")

        # RAG: Retrieve relevant context from PhotoMem and inject as assistant message
        try:
            logger.info("About to await photomem_client.search for RAG context")
            search_results = await photomem_client.search(
                new_message.text_content,
                user_id=RAG_USER_ID,
            )
            logger.info(f"photomem_client.search returned: {search_results}")
            if search_results and search_results.get('results', []):
                context_parts = []
                for result in search_results.get('results', []):
                    paragraph = result.get("memory") or result.get("text")
                    if paragraph:
                        source = "photomem Memories"
            ...
            logger.warning(f"Failed to inject RAG context from PhotoMem: {e}")
...
1. **Semantic Memory Retrieval**: Uses PhotoMem to store and retrieve contextually relevant memories
...
```sh
python photomem-livekit-voice-agent.py start
```
or to start your agent in console mode to run inside your terminal:

```sh
python photomem-livekit-voice-agent.py console
```
...
```sh
python photomem-livekit-voice-agent.py dev
```
