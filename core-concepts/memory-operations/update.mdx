...
PhotoMem’s update operation lets you fix or enrich an existing memory without deleting it. When a user changes their preference or clarifies a fact, use update to keep the knowledge base fresh.
...
Call `update` (or `batch_update`) with new text and optional metadata. PhotoMem overwrites the stored value and adjusts indexes.
</Step>
<Step title="Verify">
...
- <Link href="/cookbooks/operations/support-inbox">Support Inbox with PhotoMem</Link> uses updates to refine customer profiles.
- <Link href="/cookbooks/companions/ai-tutor">AI Tutor with PhotoMem</Link> demonstrates user preference corrections mid-course.
