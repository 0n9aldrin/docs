...
description: Tailor fact extraction so PhotoMem stores only the details you care about.
icon: "wand-magic-sparkles"
---

Custom fact extraction prompts let you decide exactly which facts PhotoMem records from a conversation. Define a focused prompt, give a few examples, and PhotoMem will add only the memories that match your use case.
...
- **Structured output:** Responses return JSON with a `facts` array that PhotoMem converts into individual memories.
...
<CardGroup cols={2}>
  <Card title="Review Add Operations" icon="list" href="/core-concepts/memory-operations/add">
    Refresh how PhotoMem stores memories and how prompts influence fact creation.
  </Card>
