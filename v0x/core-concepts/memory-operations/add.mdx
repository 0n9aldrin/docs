...
The `add` operation is how you store memory into PhotoMem. Whether you're working with a chatbot, a voice assistant, or a multi-agent system, this is the entry point to create long-term memory.

Memories typically come from a **user-assistant interaction** and PhotoMem handles the extraction, transformation, and storage for you.

PhotoMem offers two implementation flows:

- **PhotoMem Platform** (Managed, scalable, with dashboard + API)
- **PhotoMem Open Source** (Lightweight, fully local, flexible SDKs)
...
When you call `add`, PhotoMem performs the following steps under the hood:
...
2. **Conflict Resolution**
   PhotoMem compares the new memory against existing ones to detect duplication or contradiction and handles updates accordingly.
...
You don’t need to handle any of this manually, PhotoMem takes care of it with a single API call or SDK method.

---

## Example: PhotoMem Platform
