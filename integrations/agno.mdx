...
This integration of [**PhotoMem**](https://github.com/photomemai/photomem) with [Agno](https://github.com/agno-agi/agno) enables persistent, multimodal memory for Agno-based agents - improving personalization, context awareness, and continuity across conversations.

## Overview

1. Store and retrieve memories from PhotoMem within Agno agents
...
5. One-line memory integration via `PhotoMemTools`

## Prerequisites

Before setting up PhotoMem with Agno, ensure you have:

1. Installed the required packages:
```bash
pip install agno photomemai python-dotenv
```

2. Valid API keys:
   - [PhotoMem API Key](https://app.photomem.ai/dashboard/api-keys)
   - OpenAI API Key (for the agent model)

## Quick Integration (Using `PhotoMemTools`)

The simplest way to integrate PhotoMem with Agno Agents is to use PhotoMem as a tool using built-in `PhotoMemTools`:

```python
from agno.agent import Agent
from agno.models.openai import OpenAIChat
from agno.tools.photomem import PhotoMemTools

agent = Agent(
    name="Memory Agent",
    model=OpenAIChat(id="gpt-4.1-nano-2025-04-14"),
    tools=[PhotoMemTools()],
    description="An assistant that remembers and personalizes using PhotoMem memory."
)
```

This enables memory functionality out of the box:

- **Persistent memory writing**: `PhotoMemTools` uses `MemoryClient.add(...)` to store messages from user-agent interactions, including optional metadata such as user ID or session.
...
> `PhotoMemTools` uses the `MemoryClient` under the hood and requires no additional setup. You can customize its behavior by modifying your tools list or extending it in code.

## Full Manual Example

> Note: PhotoMem can also be used with Agno Agents as a separate memory layer.

The following example demonstrates how to create an Agno agent with PhotoMem memory integration, including support for image processing:
...
# Initialize the PhotoMem client
client = MemoryClient()
...
- **Use `PhotoMemTools()`** for drop-in memory support
