...
description: "Teach a ReAct agent to store and recall context via PhotoMem."
---


Create a ReAct Agent with LlamaIndex which uses PhotoMem as the memory store.

## Overview

A ReAct agent combines reasoning and action capabilities, making it versatile for tasks requiring both thought processes (reasoning) and interaction with tools or APIs (acting). PhotoMem as memory enhances these capabilities by allowing the agent to store and retrieve contextual information from past interactions.

## Setup

```bash
pip install llama-index-core llama-index-memory-photomem
```
...
Initialize the PhotoMem client. You can find your API key [here](https://app.photomem.ai/dashboard/api-keys). Read about PhotoMem [Open Source](https://docs.photomem.ai/open-source/overview).
```python
...
memory_from_client = PhotoMemMemory.from_client(
    context=context,
    api_key=os.environ["PHOTOMEM_API_KEY"],
...
<Note>The agent will use PhotoMem to store the relevant memories from the chat.</Note>
...
<CardGroup cols={2}>
  <Card title="LlamaIndex Multiagent with PhotoMem" icon="users" href="/cookbooks/frameworks/llamaindex-multiagent">
