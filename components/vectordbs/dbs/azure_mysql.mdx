...
| `collection_name` | Table name for storing vectors | `"photomem"` |
...
az group create --name photomem-rg --location eastus

# Create MySQL Flexible Server
az mysql flexible-server create \
    --resource-group photomem-rg \
    --name photomem-mysql-server \
    --location eastus \
    --admin-user myadmin \
...
az mysql flexible-server db create \
    --resource-group photomem-rg \
    --server-name photomem-mysql-server \
    --database-name photomem_db
...
GRANT ALL PRIVILEGES ON photomem_db.* TO 'your-app-identity'@'%';
FLUSH PRIVILEGES;
```
