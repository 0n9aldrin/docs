---
title: Memory Types
description: "See how PhotoMem layers conversation, session, and user memories to keep agents contextual."
icon: "tag"
iconType: "solid"
---

# How PhotoMem Organizes Memory

PhotoMem separates memory into layers so agents remember the right detail at the right time. Think of it like a notebook: a sticky note for the current task, a daily journal for the session, and an archive for everything a user has shared.
...
PhotoMem maps these classic categories onto its layered storage so you can decide what should fade quickly versus what should last for months.

## How does it work?

PhotoMem stores each layer separately and merges them when you query:
...
<Warning>
  Avoid storing secrets or unredacted PII in user or org memories—PhotoMem is retrievable by design. Encrypt or hash sensitive values first.
</Warning>
...
- <Link href="/cookbooks/companions/ai-tutor">AI Tutor with PhotoMem</Link> shows session vs user memories in action.
- <Link href="/cookbooks/operations/support-inbox">Support Inbox with PhotoMem</Link> demonstrates shared org memory.
