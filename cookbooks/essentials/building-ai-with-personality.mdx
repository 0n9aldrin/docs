...
Unlike other memory APIs, we built one that allows you the flexibility to store memory from all sources. User and agent memories in PhotoMem are handled by:
...
sarah_history = photomem_client.search(
...
coaching_context = photomem_client.search(
...
sarah_context = photomem_client.search(
...
coach_personality = photomem_client.search(
...
mike_context = photomem_client.search(
...
```python
# Get Mike's memories
mike_memories = photomem_client.get_all(filters={"user_id": "mike"})

# Get Sarah's memories
sarah_memories = photomem_client.get_all(filters={"user_id": "sarah"})
...
With PhotoMem's **`user_id`** and **`agent_id`** parameters, you can build AI agents that maintain consistent personalities across all users while remembering individual user data privately. Store relationship preferences with **`user_id`** and track agent context in metadata.
