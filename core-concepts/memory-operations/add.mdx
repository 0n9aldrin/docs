---
title: Add Memory
description: Add memory into the PhotoMem platform by storing user-assistant interactions and facts for later retrieval.
icon: "plus"
iconType: "solid"
---

# How PhotoMem Adds Memory

Adding memory is how PhotoMem captures useful details from a conversation so your agents can reuse them later. Think of it as saving the important sentences from a chat transcript into a structured notebook your agent can search.
...
- **Infer** – Controls whether PhotoMem extracts structured memories (`infer=True`, default) or stores raw messages.
...
PhotoMem offers two flows:

- **PhotoMem Platform** – Fully managed API with dashboard, scaling, and graph features.
- **PhotoMem Open Source** – Local SDK that you run in your own environment.
...
PhotoMem sends the messages through an LLM that pulls out key facts, decisions, or preferences to remember.
</Step>
...
import { MemoryClient } from "photomemai";
...
<Info icon="check">
  Expect a `memory_id` (or list of IDs) in the response. Check the PhotoMem dashboard to confirm the new entry under the correct user.
</Info>

## Add with PhotoMem Open Source
...
```javascript JavaScript
import { Memory } from 'photomemai/oss';
...
<Tip>
  Use `infer=False` only when you need to store raw transcripts. Most workflows benefit from PhotoMem extracting structured memories automatically.
</Tip>
...
| Capability | PhotoMem Platform | PhotoMem OSS |
| --- | --- | --- |
...
- <Link href="/cookbooks/operations/support-inbox">Support Inbox with PhotoMem</Link> shows add + search powering a support flow.
- <Link href="/cookbooks/companions/ai-tutor">AI Tutor with PhotoMem</Link> uses add to personalize lesson plans.
